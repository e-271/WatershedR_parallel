...
#' @param l2_prior_parameter Defines the L2 (gaussian) distribution that acts 
#'   as a prior on the parameters defining the conditional random field \code{P(Z|G)}. 
#'   Either a single float (applied to all outlier dimensions) or a numeric vector of 
#'   length \code{number_dimensions} giving a separate L2 prior for each dimension of 
#'   the input omes (analogous to passing a vector for \code{dirichlet_prior_parameter}). 
#'   If set to NULL, Watershed will run a grid search on held-out data to select a single 
#'   optimal L2 prior shared across dimensions. Default: 0.1
...
learn_watershed_model_parameters_from_training_data <- function(training_input_file, 
	...
	if (is.null(lambda_init)) {
		cat(paste0(nfolds,"-fold cross validation on GAM yielded optimal lambda of ", gam_data$lambda[1], "\n"))
	}
